...
<div class="text-foreground max-w-4xl my-10 mx-auto px-4">
    <span class="inline-flex justify-between gap-[2ch] w-full">
        ...
        {#if !data.tier}
            <a class="underline" href="/auth/login">Sign in with Patreon</a>
        {:else}
    ...
    {#if data.error}
        <p class="text-red-500 px-2 pb-2">Login failed ({data.error}).</p>
    {/if}
    ...
    <main class="border p-4">
        ...
        <div class="py-4">
            {#if data.tier === 'free'}
                ...
                <a class="underline" href="https://www.patreon.com/cw/dinhvu" target="_blank" rel="noopener">Join on Patreon</a>
