...
class GameManager {
    ...
    init = (
        ...
        const extraTime = settingsManager.settings.extraTime.value ? 2 * 60 : 0

        const threeMinutes = 3 * 60 + extraTime
        const fourMinutes = 4 * 60 + extraTime
        ...
        if (override) {
            ...
            this.letters = weekDayMap[day].generateBoard(
                this.dateKey,
                this.gridSize
            )
    ...
    getSearchArea = (): [number, string][] => {
        if (this.currentChain.length === 0) {
            return this.letters.entries().toArray()
        }
